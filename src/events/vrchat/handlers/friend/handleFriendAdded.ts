...
export async function handleFriendAdd(content: any) {
  ...
  if (!vrcAccount) {
    console.log(
      "[Friend Add] No VRChatAccount found for VRChat user:",
      vrcUserId,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `[Friend Add] Account ${vrcUserId} successfully verified and marked as VERIFIED`,
    );
  ...
  const user = await prisma.user.findUnique({
    where: { id: vrcAccount.userId },
  });
  if (user && user.discordId) {
    console.log(
      `[Friend Add] VRChat account for Discord user: ${user.discordId}`,
    );
    try {
      // For verified accounts, sync and publish with roles
      if (vrchatUsername) {
        // If username was fetched, it was verified
      ...
      console.warn(
        `[Friend Add] Failed to update whitelist for user ${user.discordId}:`,
        e,
      );
    }
 ...
 */
async function sendDMConfirmation(
  userId: number,
  vrcUserId: string,
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[DM Confirmation] Successfully sent verification confirmation DM to ${user.discordId}`,
      );
    } catch (dmError: any) {
      // If DM fails (user has DMs disabled, etc.), log but don't throw
      console.warn(
        `[DM Confirmation] Failed to send DM to ${user.discordId}:`,
