...
interface FriendActiveContent {
  userId?: string;
}

export async function handleFriendActive(content: unknown) {
  const typedContent = content as FriendActiveContent;
  ...
  loggers.vrchat.debug(`Upserted friend active: ${typedContent.userId}`);
}
