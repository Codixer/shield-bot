import { ButtonInteraction, MessageFlags, ButtonBuilder, ButtonStyle, ContainerBuilder, SectionBuilder, TextDisplayBuilder, ActionRowBuilder, SeparatorBuilder, SeparatorSpacingSize, type MessageActionRowComponentBuilder } from "discord.js";
...
export class VRCAccountManagerButtonHandler {
    ...
    async handleAccountManager(interaction: ButtonInteraction) {
        ...
        try {
            ...
            const vrcAccount = user.vrchatAccounts.find((acc: any) => acc.vrcUserId === vrcUserId);
            if (!vrcAccount) {
                await interaction.reply({
                    content: "❌ VRChat account not found or not linked to your Discord account.",
                    flags: MessageFlags.Ephemeral
                });
                return;
            }
    ...
    private async handleSetMain(interaction: ButtonInteraction, user: any, vrcAccount: any, vrcUserId: string) {
        // Check if user already has a MAIN account
        const currentMain = user.vrchatAccounts.find((acc: any) => acc.accountType === "MAIN");
    ...
    private async handleSetAlt(interaction: ButtonInteraction, vrcAccount: any, vrcUserId: string) {
    ...
    private async handleDelete(interaction: ButtonInteraction, vrcAccount: any, vrcUserId: string) {
        try {
            // Try to unfriend the user from VRChat
            try {
                await unfriendUser(vrcUserId);
            } catch (unfriendError) {
                console.warn(`Failed to unfriend VRChat user ${vrcUserId}:`, unfriendError);
            ...
            await interaction.reply({
                content: "❌ An error occurred while deleting the account. The account may have been partially removed.",
                flags: MessageFlags.Ephemeral
            });
        }
    }
