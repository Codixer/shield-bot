...
import { CommandInteraction, MessageFlags, EmbedBuilder, InteractionContextType, ApplicationIntegrationType } from "discord.js";
...
export class VRChatAttendanceListCommand {
  ...
  async list(interaction: CommandInteraction) {
    const user = await attendanceManager.findOrCreateUserByDiscordId(interaction.user.id);
    ...
    const activeEventId = await attendanceManager.getActiveEventIdForUser(user.id);
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x00AE86)
      .setFooter({ text: 'Use /attendance select <event_id> to switch active event' });
