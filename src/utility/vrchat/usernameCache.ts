...
export async function updateUsernameCache(vrcUserId: string): Promise<void> {
    try {
        ...
        const shouldUpdate = !vrcAccount.usernameUpdatedAt || 
        ...
        if (!currentUsername) {
            console.warn(`[Username Cache] Could not fetch username for ${vrcUserId}`);
            return;
        }
        ...
        if (currentUsername !== vrcAccount.vrchatUsername || shouldUpdate) {
            ...
            console.log(`[Username Cache] Updated username for ${vrcUserId}: ${currentUsername}`);
        }
    } catch (error) {
        console.warn(`[Username Cache] Failed to update username for ${vrcUserId}:`, error);
    }
}
 ...
 */
export async function forceUpdateUsernameCache(vrcUserId: string): Promise<void> {
    try {
        const vrcAccount = await prisma.vRChatAccount.findFirst({
            where: { vrcUserId }
        });
        ...
        if (currentUsername) {
            ...
            console.log(`[Username Cache] Force updated username for ${vrcUserId}: ${currentUsername}`);
        }
    } catch (error) {
        console.warn(`[Username Cache] Failed to force update username for ${vrcUserId}:`, error);
    }
}
