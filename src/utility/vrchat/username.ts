 ...
 */
export async function getVRChatUsername(vrcUserId: string): Promise<string | null> {
    try {
        ...
        if (vrcAccount && vrcAccount.vrchatUsername) {
            ...
            if (vrcAccount.usernameUpdatedAt && vrcAccount.usernameUpdatedAt > oneWeekAgo) {
        ...
        console.warn(`[getVRChatUsername] Failed to get username for ${vrcUserId}:`, error);
        return null;
    }
}
 ...
 */
export async function getVRChatUsernameFresh(vrcUserId: string): Promise<string | null> {
    try {
        ...
        console.warn(`[getVRChatUsernameFresh] Failed to get fresh username for ${vrcUserId}:`, error);
        return null;
    }
}
